  ...
  return (
    <ToastContext.Provider value={{ addToast }}>
      ...
      {snackbars.map(snackbar => (
        <Snackbar
          key={snackbar.id}
          anchorOrigin={{ vertical: snackbar.options?.vertical || 'top', horizontal: snackbar.options?.horizontal || 'center' }}
          open
          autoHideDuration={snackbar.options?.autoHideDuration || 6000}
          onClose={() => removeToast(snackbar.id)}
        >
          <Alert onClose={() => removeToast(snackbar.id)} severity={snackbar.options?.type} sx={{ width: '100%' }}>
          {snackbar.message}
          </Alert>
        </Snackbar>
      ))}
